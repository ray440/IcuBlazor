...
class Automation {
    ...
    public GetContent(ekey: number, htm: boolean) {
        const e = refs.Get<HTMLElement>(ekey);
        return htm ? e.innerHTML : e.textContent;
    }
    public SetContent(ekey: number, newContent: string) {
        const e = refs.Get<HTMLElement>(ekey);
...
class UIutil {
    ...
    public GetClientRect(sel: string) {
        if (sel.length === 0) {
    ...
    public IntAlign(sel: string) {
        const e = Automation.findOne(sel) as HTMLElement;
        const b = e.getBoundingClientRect();
